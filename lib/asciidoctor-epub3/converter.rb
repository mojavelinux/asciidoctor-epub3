...
class Converter
  ...
  def convert node, name = nil
    if name == 'document'
      @validate = true if node.attr? 'ebook-validate'
      @extract = true if node.attr? 'ebook-extract'
  ...
  def inline_anchor node
    ...
    when :xref
      ...
      id_attr = unless @xrefs_seen.include? refid
        ...
        %( id="xref-#{xdoc ? (refid.sub '#', '--') : refid}")
      end
      unless (text = node.text)
        if xdoc
          xdoc_id, xdoc_refid = refid.split '#', 2
          xdoc_doc = node.document.references[:spine_items].find {|doc| xdoc_id == (doc.id || (doc.attr 'docname')) }
          if xdoc_doc
            unless (text = (xdoc_id == xdoc_refid ? xdoc_doc.doctitle : xdoc_doc.references[:ids][xdoc_refid]))
